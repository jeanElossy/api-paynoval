...
- `.env.example` est **incomplet** par rapport aux variables réellement lues. Pour la liste réelle : `grep -roE "process\.env\.[A-Z0-9_]+" src | sort -u`. Familles importantes :
  ...
  - providers : `WAVE_*`, `ORANGE_*`, `MTN_*`, `MOOV_*`, `VISA_DIRECT_*` (chacun avec `_BASE_URL`, `_API_KEY`, `_WEBHOOK_SECRET`, `_MOCK`). ⚠️ `STRIPE_*` et `BANK_GENERIC_*` étaient encore listés ici au 2026-09-09 alors que les deux rails ont été **retirés** — le rail carte est `visa_direct`, et le rail bancaire n'existe plus (`test/noBankRail.test.js`) ;
...
**L'exigence est déclarée par route, pas globalement.** `/initiate` (création + réservation de fonds) suit `IDEMPOTENCY_REQUIRED`, **aujourd'hui souple** — la variable n'est déclarée nulle part et le défaut du code vaut `false`. `/confirm`, `/cancel` et `/refund` sont marquées `required: false` **en dur** : ce sont des transitions d'état, déjà protégées par `assertTransition` et les drapeaux `fundsCaptured` / `beneficiaryCredited` / `reserveReleased` sous transaction Mongo — c'est le modèle de Wise, qui n'exige la clé que sur la création. Ne pas ajouter `IDEMPOTENCY_REQUIRED` à `.env.example` : ce fichier pilote le contrôle de présence de `dotenv-safe`, sa présence y rendrait la variable obligatoire dans le `.env` et casserait le démarrage. Le basculement se fait dans l'environnement Render. Le mobile en envoie une depuis cette version (`TransactionContext`, une clé **par intention de virement** — pas par appel, sinon elle ne protège de rien — libérée par `clearTransactionData()`). L'exigence sera activée une fois le parc à jour.
