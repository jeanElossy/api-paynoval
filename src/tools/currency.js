...
async function getRates(rawBase) {
  // Normaliser entrée
  let baseKey = String(rawBase).trim().toUpperCase();
  // Appliquer alias si présent
  let baseNorm = CURRENCY_ALIASES[baseKey] || baseKey;
  // Retirer tout sauf A-Z
  baseNorm = baseNorm.replace(/[^A-Z]/g, '');
...
async function convertAmount(from, to, amount) {
  ...
  // Normaliser et alias source
  let fromKey = String(from).trim().toUpperCase();
  let fromCode = CURRENCY_ALIASES[fromKey] || fromKey;
  fromCode = fromCode.replace(/[^A-Z]/g, '');

  // Normaliser et alias cible
  let toKey = String(to).trim().toUpperCase();
  let toCode = CURRENCY_ALIASES[toKey] || toKey;
  toCode = toCode.replace(/[^A-Z]/g, '');
