 ...
 * - Annulation / remboursement / actions admin : gestion avancée.
 *
 * ✅ Ajout (format standard unique) :
 * - amountSource, currencySource
 * - amountTarget, currencyTarget
 * - feeSource
 * - fxRateSourceToTarget
 * - money.source, money.feeSource, money.target
 */
...
const { notifyTransactionViaGateway } = require("../services/notifyGateway");
const { convertAmount } = require("../tools/currency");
const { normCur } = require("../utils/currency"); // ✅ NEW
const generateTransactionRef = require("../utils/generateRef");
...
async function notifyParties(tx, status, session, senderCurrencySymbol) {
  try {
    ...
    const dateStr = new Date().toLocaleString("fr-FR");
    const token = tx.verificationToken ? String(tx.verificationToken) : "";
    const webLink = token ? `${PRINCIPAL_URL}/confirm/${tx._id}?token=${encodeURIComponent(token)}` : `${PRINCIPAL_URL}/confirm/${tx._id}`;
    const mobileLink = token ? `paynoval://confirm/${tx._id}?token=${encodeURIComponent(token)}` : `paynoval://confirm/${tx._id}`;
    ...
    const shouldEmailSender = sEmailChan && ((sTypeKey === "txSent" && sTxSentType) || (sTypeKey === "txFailed" && sTxFailedType));
    const shouldEmailReceiver = rEmailChan && ((rTypeKey === "txReceived" && rTxReceivedType) || (rTypeKey === "txFailed" && rTxFailedType));
...
exports.confirmController = async (req, res, next) => {
  ...
  try {
    ...
    const ok = looksLikeSha256Hex(stored) ? sha256Hex(sanitizedCode) === stored : sanitizedCode === stored;
    ...
    if (!creditAmount || !Number.isFinite(creditAmount) || creditAmount <= 0) {
      const snap = tx.feeSnapshot || {};
      const snapLocal = snap.convertedNetAfterFees ?? snap.convertedNet ?? snap.convertedNetAfterFee ?? null;
      const v = toFloat(snapLocal, null);
      if (Number.isFinite(v) && v > 0) creditAmount = v;
    }
    ...
    // ✅ force legacy localAmount cohérent
    if (!tx.localAmount || round2(toFloat(tx.localAmount)) !== creditAmount) {
      tx.localAmount = dec2(creditAmount);
    }

    // ✅ standard fields (toujours)
    ...
    const fxRate = round2(toFloat(tx.exchangeRate, 1));
    ...
    tx.fxRateSourceToTarget = mongoose.Types.Decimal128.fromString(String(toFloat(tx.exchangeRate, 1)));

    tx.money = {
      source: { amount: amountSourceStd, currency: currencySourceISO },
      feeSource: { amount: feeSourceStd, currency: currencySourceISO },
      target: { amount: amountTargetStd, currency: currencyTargetISO },
      fxRateSourceToTarget: toFloat(tx.exchangeRate, 1),
    };
...
exports.refundController = async (req, res, next) => {
  ...
  try {
    ...
    logger.warn(`[ALERTE REFUND] Remboursement manuel ! tx=${transactionId}, by=${req.user?.email || req.user?.id}, amount=${amt}`);
...
exports.reassignController = async (req, res, next) => {
  ...
  try {
    ...
    logger.warn(`ALERTE REASSIGN: tx=${transactionId} réassignée par ${req.user?.email || req.user?.id} à ${cleanNewEmail}`);
