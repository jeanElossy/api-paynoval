...
exports.settleCagnotteClosureFees = asyncHandler(async (req, res) => {
  ...
  if (!ref || !idem || !cId || !vId || !initiatorId) {
    return res.status(400).json({
      success: false,
      error:
        "reference, idempotencyKey, cagnotteId, vaultId et initiatedByUserId sont requis.",
    });
  }
  ...
  try {
    treasuryMeta = resolveCagnotteTreasuryMeta({
      treasuryUserId,
      treasurySystemType,
      treasuryLabel,
    });
  ...
  if (existingByReference) {
    return res.status(200).json({
      ...
      reference: existingByReference.reference,
      newBalance:
        existingByReference?.treasuryWalletAfter?.availableAmount ??
        existingByReference?.treasuryWalletAfter?.amount ??
        null,
      data: existingByReference,
    });
  }
  ...
  if (existingByIdem) {
    return res.status(200).json({
      ...
      reference: existingByIdem.reference,
      newBalance:
        existingByIdem?.treasuryWalletAfter?.availableAmount ??
        existingByIdem?.treasuryWalletAfter?.amount ??
        null,
      data: existingByIdem,
    });
  }
  ...
  try {
    ...
    return res.status(201).json({
      ...
      reference: settlement.reference,
      newBalance:
        settlement?.treasuryWalletAfter?.availableAmount ??
        settlement?.treasuryWalletAfter?.amount ??
        null,
      data: settlement.toObject ? settlement.toObject() : settlement,
    });
  } catch (err) {
