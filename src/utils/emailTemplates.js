...
const commonStyles = `
  <style>
    ...
    .header img { display:block; margin-right:30px; height:60px; }
    .header h1 { margin-top: 16; color:#fff; font-size:30px;}
    ...
    .notice { background-color:#e8f5e9; border-left:4px solid #43a047; padding:15px; font-size: 18px; color:#2e7d32; margin-bottom:15px; }
    h1 { margin:0; }
...
function initiatedSenderTemplate(data) {
  ...
  `;
  const bodyHtml = `
    <p style="font-size:24px;margin-bottom:20px;">Bonjour ${data.name || 'utilisateur'},</p>
    <p style="margin-bottom:24px; font-size:22px;">Votre transaction a été initiée avec succès. Les fonds seront débloqués une fois que le destinataire aura validé.</p>
    ...
    <p style="margin-bottom:25px; font-size:23px;">Vos transactions en attente seront automatiquement annulées au bout de 10 jours.</p>
...
function initiatedReceiverTemplate(data) {
  ...
  `;
  const bodyHtml = `
    <p style="font-size:24px;margin-bottom:20px;">Bonjour ${data.name},</p>
    <p style="margin-bottom:25px;font-size:22px;">Vous avez reçu une transaction en attente de validation.</p>
    ${detailsTableHtml(rows)}
...
function confirmedSenderTemplate(data) {
  ...
  const bodyHtml = `
    <p style="font-size:24px;margin-bottom:20px;">Bonjour ${data.name},</p>
    <p style="margin-bottom:25px; font-size:22px;">Votre transaction a été validée par le destinataire.</p>
    ${detailsTableHtml(rows)}
...
function confirmedReceiverTemplate(data) {
  ...
  const bodyHtml = `
    <p style="font-size:24px;margin-bottom:20px;">Bonjour ${data.name},</p>
    <p style="margin-bottom:25px; font-size:22px;">Vous avez validé la transaction avec succès.</p>
    ${detailsTableHtml(rows)}
...
function cancelledSenderTemplate(data) {
  ...
  const bodyHtml = `
    <p style="font-size:24px;margin-bottom:20px;">Bonjour ${data.name},</p>
    <p style="margin-bottom:25px;font-size:22px;">Votre transaction a été annulée${data.reason ? ` : ${data.reason}` : '.'}</p>
    ${detailsTableHtml(rows)}
...
function cancelledReceiverTemplate(data) {
  ...
  const bodyHtml = `
    <p style="font-size:24px;margin-bottom:20px;">Bonjour ${data.name},</p>
    <p style="margin-bottom:25px;font-size:22px;">La transaction a été annulée${data.reason ? ` : ${data.reason}` : '.'}</p>
    ${detailsTableHtml(rows)}
