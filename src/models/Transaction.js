...
module.exports = conn => {
  ...
  const transactionSchema = new Schema({
    ...
    amount:    { type: Schema.Types.Decimal128, required: true, validate: { validator: v => parseFloat(v.toString()) >= 0.01, message: props => `Le montant doit être ≥ 0.01, reçu ${props.value}` } },
    transactionFees: { type: Schema.Types.Decimal128, required: true, default: mongoose.Types.Decimal128.fromString('0.00'), validate: { validator: v => parseFloat(v.toString()) >= 0.00, message: props => `Les frais doivent être ≥ 0.00, reçus ${props.value}` } },
    netAmount: { type: Schema.Types.Decimal128, required: true, validate: { validator: v => parseFloat(v.toString()) >= 0.00, message: props => `Le net à créditer doit être ≥ 0.00, reçu ${props.value}` } },
    ...
    localAmount:       { type: Schema.Types.Decimal128, required: true },
