...
function okResult({ providerReference, externalStatus = "PENDING", message, raw }) {
  return {
    ok: true,
    provider: PROVIDER,
    providerReference: providerReference || buildRef("MTN"),
    externalStatus,
...
async function payout(input = {}) {
  ...
  const payload = {
    reference:
      ...
      buildRef("PNV_MTN_OUT"),
    amount: Number(input.amount || 0),
    currency: upper(input.currency || "XOF"),
    msisdn: input.recipient?.phone || input.phone || null,
    customerName: input.recipient?.name || null,
    description: input.description || "PayNoval MTN payout",
  ...
  if (cfg.mock || !cfg.baseURL) {
    return okResult({
      ...
      message: "MTN mock payout accepted",
  ...
  try {
    ...
    return okResult({
      providerReference:
        data.providerReference || data.reference || data.id || buildRef("MTN_PAYOUT"),
      externalStatus: data.status || "PENDING",
      message: data.message || "MTN payout accepted",
      raw: data,
    });
  } catch (err) {
    return failResult({
      ...
      errorCode: err?.response?.data?.code || err.code || "MTN_PAYOUT_ERROR",
      errorMessage: err?.response?.data?.message || err.message || "MTN payout failed",
...
async function collect(input = {}) {
  ...
  const payload = {
    ...
    amount: Number(input.amount || 0),
    currency: upper(input.currency || "XOF"),
    msisdn: input.sender?.phone || input.phone || null,
    customerName: input.sender?.name || null,
    description: input.description || "PayNoval MTN collection",
    metadata: input.metadata || {},
  };
  ...
  if (cfg.mock || !cfg.baseURL) {
    return okResult({
      ...
      externalStatus: "PENDING",
      message: "MTN mock collection initiated",
      raw: { mock: true, payload },
    });
  }
  ...
  try {
    ...
    return okResult({
      providerReference:
        data.providerReference || data.reference || data.id || buildRef("MTN_COLLECT"),
      externalStatus: data.status || "PENDING",
      message: data.message || "MTN collection initiated",
      raw: data,
    });
  } catch (err) {
    return failResult({
      providerReference: err?.response?.data?.providerReference || null,
      externalStatus: err?.response?.data?.status || "FAILED",
      errorCode: err?.response?.data?.code || err.code || "MTN_COLLECT_ERROR",
      errorMessage:
        err?.response?.data?.message || err.message || "MTN collection failed",
      raw: err?.response?.data || {},
    });
  }
}
...
async function parseWebhook(req = {}) {
  ...
  const verification = verifyHmacWebhook({
    ...
    signatureHeaders: ["x-mtn-signature", "x-signature"],
    timestampHeaders: ["x-mtn-timestamp", "x-timestamp"],
    algorithm: "sha256",
    toleranceSeconds: Number(process.env.MTN_WEBHOOK_TOLERANCE_SEC || 300),
  });
