"use strict";

const axios = require("axios");
const crypto = require("crypto");
const { verifyHmacWebhook } = require("../shared/webhookSecurity");

const PROVIDER = "wave";

function norm(v) {
  return String(v || "").trim();
}

function upper(v) {
  return norm(v).toUpperCase();
}

function nowIso() {
  return new Date().toISOString();
}

function buildRef(prefix = "WAVE") {
  return `${prefix}_${Date.now()}_${crypto.randomBytes(4).toString("hex")}`;
}

function canonicalStatus(status) {
  switch (upper(status)) {
    case "SUCCESS":
    case "SUCCEEDED":
    case "COMPLETED":
    case "PAID":
    case "OK":
      return "completed";

    case "PENDING":
    case "PROCESSING":
    case "IN_PROGRESS":
    case "ACCEPTED":
    case "INITIATED":
      return "processing";

    case "FAILED":
    case "ERROR":
    case "REJECTED":
    case "DECLINED":
      return "failed";

    case "CANCELLED":
    case "CANCELED":
    case "VOIDED":
    case "EXPIRED":
      return "cancelled";

    default:
      return "pending";
  }
}

function getConfig() {
  return {
    baseURL: process.env.WAVE_BASE_URL || "",
    apiKey: process.env.WAVE_API_KEY || "",
    webhookSecret: process.env.WAVE_WEBHOOK_SECRET || "",
    timeout: Number(process.env.WAVE_TIMEOUT_MS || 15000),
    mock: String(process.env.WAVE_MOCK || "true").toLowerCase() === "true",
  };
}

function buildHeaders() {
  const cfg = getConfig();
  return {
    "Content-Type": "application/json",
    Accept: "application/json",
    ...(cfg.apiKey ? { Authorization: `Bearer ${cfg.apiKey}` } : {}),
  };
}

function okResult({ providerReference, externalStatus = "PENDING", message, raw }) {
  return {
    ok: true,
    provider: PROVIDER,
    providerReference: providerReference || buildRef("WAVE"),
    externalStatus,
    status: canonicalStatus(externalStatus),
    message: message || "Accepted by provider",
    raw: raw || {},
  };
}

function failResult({
  providerReference = null,
  externalStatus = "FAILED",
  errorCode = "PROVIDER_ERROR",
  errorMessage = "Provider request failed",
  raw = {},
}) {
  return {
    ok: false,
    provider: PROVIDER,
    providerReference,
    externalStatus,
    status: canonicalStatus(externalStatus),
    errorCode,
    errorMessage,
    raw,
  };
}

function buildPayoutPayload(input = {}) {
  return {
    reference:
      input.txReference ||
      input.reference ||
      input.idempotencyKey ||
      buildRef("PNV_WAVE_OUT"),
    amount: Number(input.amount || 0),
    currency: upper(input.currency || "XOF"),
    phoneNumber: input.recipient?.phone || input.phone || null,
    recipientName: input.recipient?.name || input.customerName || null,
    description: input.description || "PayNoval Wave payout",
    country: input.country || null,
    operator: input.operator || "wave",
    metadata: input.metadata || {},
  };
}

function buildCollectPayload(input = {}) {
  return {
    reference:
      input.txReference ||
      input.reference ||
      input.idempotencyKey ||
      buildRef("PNV_WAVE_IN"),
    amount: Number(input.amount || 0),
    currency: upper(input.currency || "XOF"),
    phoneNumber: input.sender?.phone || input.phone || null,
    customerName: input.sender?.name || input.customerName || null,
    description: input.description || "PayNoval Wave collection",
    country: input.country || null,
    operator: input.operator || "wave",
    metadata: input.metadata || {},
  };
}

async function payout(input = {}) {
  const cfg = getConfig();
  const payload = buildPayoutPayload(input);

  if (cfg.mock || !cfg.baseURL) {
    return okResult({
      providerReference: buildRef("WAVE_PAYOUT"),
      externalStatus: "PENDING",
      message: "Wave mock payout accepted",
      raw: { mock: true, createdAt: nowIso(), payload },
    });
  }

  try {
    const res = await axios.post(`${cfg.baseURL}/payouts`, payload, {
      headers: buildHeaders(),
      timeout: cfg.timeout,
    });

    const data = res?.data || {};
    return okResult({
      providerReference:
        data.providerReference || data.reference || data.id || buildRef("WAVE_PAYOUT"),
      externalStatus: data.status || "PENDING",
      message: data.message || "Wave payout accepted",
      raw: data,
    });
  } catch (err) {
    return failResult({
      providerReference: err?.response?.data?.providerReference || null,
      externalStatus: err?.response?.data?.status || "FAILED",
      errorCode: err?.response?.data?.code || err.code || "WAVE_PAYOUT_ERROR",
      errorMessage:
        err?.response?.data?.message || err.message || "Wave payout failed",
      raw: err?.response?.data || {},
    });
  }
}

async function collect(input = {}) {
  const cfg = getConfig();
  const payload = buildCollectPayload(input);

  if (cfg.mock || !cfg.baseURL) {
    return okResult({
      providerReference: buildRef("WAVE_COLLECT"),
      externalStatus: "PENDING",
      message: "Wave mock collection initiated",
      raw: { mock: true, createdAt: nowIso(), payload },
    });
  }

  try {
    const res = await axios.post(`${cfg.baseURL}/collections`, payload, {
      headers: buildHeaders(),
      timeout: cfg.timeout,
    });

    const data = res?.data || {};
    return okResult({
      providerReference:
        data.providerReference || data.reference || data.id || buildRef("WAVE_COLLECT"),
      externalStatus: data.status || "PENDING",
      message: data.message || "Wave collection initiated",
      raw: data,
    });
  } catch (err) {
    return failResult({
      providerReference: err?.response?.data?.providerReference || null,
      externalStatus: err?.response?.data?.status || "FAILED",
      errorCode: err?.response?.data?.code || err.code || "WAVE_COLLECT_ERROR",
      errorMessage:
        err?.response?.data?.message || err.message || "Wave collection failed",
      raw: err?.response?.data || {},
    });
  }
}

async function parseWebhook(req = {}) {
  const body = req.body || {};

  const verification = verifyHmacWebhook({
    req,
    secret: process.env.WAVE_WEBHOOK_SECRET || "",
    signatureHeaders: ["x-wave-signature", "x-signature"],
    timestampHeaders: ["x-wave-timestamp", "x-timestamp"],
    algorithm: "sha256",
    toleranceSeconds: Number(process.env.WAVE_WEBHOOK_TOLERANCE_SEC || 300),
  });

  const providerReference =
    body.providerReference ||
    body.reference ||
    body.transactionId ||
    body.id ||
    null;

  const txReference =
    body.txReference ||
    body.merchantReference ||
    body.clientReference ||
    body.metadata?.txReference ||
    body.metadata?.txCoreReference ||
    null;

  const externalStatus =
    body.status ||
    body.transactionStatus ||
    body.eventStatus ||
    body.state ||
    "PENDING";

  return {
    provider: PROVIDER,
    verified: verification.verified,
    verificationReason: verification.reason,
    eventId: body.eventId || body.id || providerReference || null,
    eventType: body.type || body.eventType || body.event || null,
    providerReference,
    txReference,
    externalStatus,
    status: canonicalStatus(externalStatus),
    amount: Number(body.amount || body.value || 0),
    currency: upper(body.currency || "XOF"),
    raw: body,
  };
}

module.exports = {
  provider: PROVIDER,
  payout,
  collect,
  parseWebhook,
  mapStatus: canonicalStatus,
};